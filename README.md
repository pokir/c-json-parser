...
Copy the `json_parser.c` and `json_parser.h` files to your project.

Example:

```c
#include "path/to/json_parser.h"
#include <stdio.h>
...
int main() {
    ...
    bool success = parse_json(
        array_json,
        "[{\"key1\": \"value1\", \"key2\": true}, 12345, 0.6789, null, true, false]"
    );
...
```sh
make test
cd target # so executables can have access to the resource folder
```

Then run the files in the `target` folder.
